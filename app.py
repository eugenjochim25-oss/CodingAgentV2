...
from routes.api_routes import create_api_router

config = Config()
config_status = config.validate()

app = FastAPI(title="CodingAgentV2", version="2.0")
...
# Store config in app for services
app.state.ai_config = config_status.get('ai_config', {})
...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        self.ai_config = app.state.ai_config
...
async def handle_live_code_execution(data: dict, client_id: str):
    ...
    try:
        # Get AI config from manager
        ai_config = manager.ai_config
...
# Make manager available app-wide
...
async def startup_event():
    ...
    log_config.setup_logging(config.LOG_LEVEL)
